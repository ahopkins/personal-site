...
const nodes = (markup) => {
    return _nodes(unified().use(html).parse(markup).children[0].children[1].children[0])
}
...
function attacher(options) {
    ...
    function blockTokenizer(eat, value, silent) {
        ...
        const iconContainerNodes =
            config.icons === "none" ? [] : [element("span", "icon", [iconNodes])]

        // build the nodes for the full markup
        const admonition = element(
            "div",
            ["notification", `is-${keyword}`],
            [
                element("div", "heading", [
                    element("span", "", iconContainerNodes.concat(titleNodes)),
                ]),
                contentNodes,
            ],
        )
